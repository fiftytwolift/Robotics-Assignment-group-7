...
for time=dt_PID:dt_PID:tfinal
    ...
    q1_e_diff(i) = (q1dot_ref(ceil(i/(dt/dt_PID)))-q1dot_ref(max(ceil((i-1)/(dt/dt_PID)),1)))/dt/180*pi-(q1_diff(i)-q1_diff(max(i-1,1)))/dt_PID;
    q2_e_diff(i) = (q2dot_ref(ceil(i/(dt/dt_PID)))-q2dot_ref(max(ceil((i-1)/(dt/dt_PID)),1)))/dt/180*pi-(q2_diff(i)-q2_diff(max(i-1,1)))/dt_PID;
