...
for time_index = 1: length(time_control)
    ...
    if time_index==1 || time_index == length(time_control)
        ...
        qdot_buffer = round(double(subs(J_v_xy_inv,[q1 q2],[q1_ref(time_index)/180*pi q2_ref(time_index)/180*pi])),3)*...
            [xdot_ref(time_index);ydot_ref(time_index)];
    end
...
for time=dt_PID:dt_PID:tfinal
    ...
    q1_diff(i) = q1dot_ref(ceil(i/(dt/dt_PID))) - qdot(1);
    q2_diff(i) = q2dot_ref(ceil(i/(dt/dt_PID))) - qdot(2);
...
for time_index = 1: length(time_control)
    ...
    xdot_ref_b(time_index) = [3*time_control(time_index)^2 2*time_control(time_index) 1 0]*x_eq_coeff;
    ydot_ref_b(time_index) = [3*time_control(time_index)^2 2*time_control(time_index) 1 0]*y_eq_coeff;
    % convert the taskspace reference into jointspace reference
    [q1_buffer, q2_buffer] =  two_arm_IK(x_ref_b(time_index),y_ref_b(time_index),false);
    q1_ref_b(time_index) = q1_buffer;
    q2_ref_b(time_index) = q2_buffer;
    if time_index==1 || time_index == length(time_control)
        qdot_buffer = [0,0];
    else
        qdot_buffer = round(double(subs(J_v_xy_inv,[q1 q2],[q1_ref_b(time_index)/180*pi q2_ref_b(time_index)/180*pi])),3)*...
            [xdot_ref_b(time_index);ydot_ref_b(time_index)];
    end
    q1dot_ref_b(time_index) = round(double(qdot_buffer(1)),4);
    q2dot_ref_b(time_index) = round(double(qdot_buffer(2)),4);
end
toc;
fprintf('finished calculating reference in joint space\n')
%% PD controller
...
for time=dt_PID:dt_PID:tfinal
    % Error term in angular velocity
    q1_diff(i) = q1dot_ref_b(ceil(i/(dt/dt_PID))) - qdot(1);
    q2_diff(i) = q2dot_ref_b(ceil(i/(dt/dt_PID))) - qdot(2);
    
    % Error term in angular acceleration
    q1_e_diff(i) = (q1dot_ref_b(ceil(i/(dt/dt_PID)))-q1dot_ref_b(max(ceil((i-1)/(dt/dt_PID)),1)))/dt-(q1_diff(i)-q1_diff(max(i-1,1)))/dt_PID;
    q2_e_diff(i) = (q2dot_ref_b(ceil(i/(dt/dt_PID)))-q2dot_ref_b(max(ceil((i-1)/(dt/dt_PID)),1)))/dt-(q2_diff(i)-q2_diff(max(i-1,1)))/dt_PID;
